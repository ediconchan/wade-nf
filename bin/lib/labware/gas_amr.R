# ============================================================================
# LABWARE_GAS_AMR -- port of R/LabwareUpload_GAS_AMR.R : labware_gas_amr()
# ----------------------------------------------------------------------------
# Consumes the collated output_profile_GAS_AMR.csv and writes the LabWare AMR
# upload CSV (+ _bad split). Runs once after COLLATE; the sample loop over the
# aggregated table is intrinsic and preserved. MIC / interpretation logic kept
# verbatim. Only the I/O plumbing (explicit profile + outdir) is refactored.
# ============================================================================

run_labware_gas_amr <- function(profile, outdir) {
  outdir <- sub("/?$", "/", outdir)
    ...
    }else
    {
      ...
      if (lw_ermA == "POS")
      {
        ...
        } else
        {
          ...
          {
            molec_profile_ermA <- "ermA S N90H/D97G"
      ...
      msrD <- posneg_gene(Output.df, "msrD", m)
  ...
  write.csv(lw_Output.df, paste0(outdir, "LabWareUpload_GAS_AMR.csv"), quote = FALSE,  row.names = FALSE)
  write.csv(lw_output_bad.df, paste0(outdir, "LabWareUpload_GAS_AMR_bad.csv"), quote = FALSE,  row.names = FALSE)
