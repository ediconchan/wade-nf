# ============================================================================
# LABWARE_PNEUMO_AMR -- port of R/LabwareUpload_PNEUMO_AMR.R : labware_pneumo_amr()
# ----------------------------------------------------------------------------
# Joins the collated AMR profile with the 23S rRNA counts, computes predicted
# MICs via regression + MIC_table.csv (MICcalc), and writes the LabWare AMR
# upload (+ _good/_bad). MIC formulas/interpretations kept verbatim. Plumbing:
#   amr_profile  = collated output_profile_PNEUMO_AMR.csv
#   rrna23s      = collated output_profile_PNEUMO_rRNA23S.csv
#   refdir       = wade-data/PNEUMO/AMR  (reads reference/MIC_table.csv)
# ============================================================================

run_labware_pneumo_amr <- function(amr_profile, rrna23s, refdir, outdir) {
  outdir <- sub("/?$", "/", outdir)
  refdir <- sub("/?$", "/", refdir)
  dir.create(outdir, showWarnings = FALSE, recursive = TRUE)

  AMR_Output.df <- as_tibble(read.csv(amr_profile, header = TRUE, sep = ",", stringsAsFactors = FALSE))
  rRNA23S_Output.df <- as_tibble(read.csv(rrna23s, header = TRUE, sep = ",", stringsAsFactors = FALSE))
  Combined_Output.df <- full_join(AMR_Output.df, rRNA23S_Output.df, by = "SampleNo")

  NumSamples <- dim(Combined_Output.df)[1]

  MIC_table <- as_tibble(read.csv(paste0(refdir, "reference/MIC_table.csv"),
  ...
  for (m in 1L:NumSamples)
  {
    ...
    } else
    {
      ...
      {
        lev <- tibble(lev_MIC = "Error", lev_interp = "Error", lev_profile = "Error")
      }else
      {
        lev_MIC_calc <-  round(-0.218 +
                              (gyrA$v_gyrA_S81F * 2.028)+ (gyrA$v_gyrA_S81Y * 1.564)+
                              (gyrA$v_gyrA_S81L * 3.564)+ (parC$v_parC_S79 * 1.654)+
      ...
      if(str_detect(gyrA$lw_gyrA, "Err") | str_detect(parC$lw_parC, "Err"))
      {
        mox <- tibble(mox_MIC = "Error", mox_interp = "Error", mox_profile = "Error")
      }else
      {
        mox_MIC_calc <-  round(-2.819 +
                              (gyrA$v_gyrA_S81F * 3.130)+ (gyrA$v_gyrA_S81Y * 3.907)+
                              (gyrA$v_gyrA_S81L * 4.907)+ (parC$v_parC_S79 * 0.911))
      ...
      {
        pen <- tibble(pen_MIC = "Error", pen_interp = "Error", pen_profile = "Error")
      }else
      {
        pen_MIC_calc <- round(-4.6099 +
                             (pbp1a$v_pbp1a_motif1 * 1.5466)+ (pbp1a$v_pbp1a_motif4 * 0.9491)+
                             (pbp2b$v_pbp2b_motif2 * 1.2025)+ (pbp2b$v_pbp2b_motif3 * 0.3563)+
                             (pbp2x$v_pbp2x_motif1_SAFK * 1.6259)+ (pbp2x$v_pbp2x_motif3_EDT * 1.5476)+
                             (pbp2x$v_pbp2x_motif3_KEA * 0.6760)+ (pbp2x$v_pbp2x_motif4_VKSG * 0.7536))
      ...
      {
        cro <- tibble(cro_MIC = "Error", cro_interp = "Error", cro_profile = "Error")
        cxm <- tibble(cxm_MIC = "Error", cxm_interp = "Error", cxm_profile = "Error")
      }else
      {
        cro_MIC_calc <- round(-2.709 +
                             (pbp1a$v_pbp1a_motif1 * 1.25)+ (pbp2x$v_pbp2x_motif1_SAFK * 2.72)+
                             (pbp2x$v_pbp2x_motif3_EDT * 0.76)+ (pbp2x$v_pbp2x_motif4_VKSG * 0.989))
        cro <- MICcalc(MIC_table, "cro", cro_MIC_calc, -3, 2L)

        cxm_MIC_calc <- round(-1.018 +
                             (pbp1a$v_pbp1a_motif1_SAMK * 1.509)+ (pbp1a$v_pbp1a_motif1_SSMK * 2.170)+
                             (pbp2x$v_pbp2x_motif1_SAFK * 2.322)+ (pbp2x$v_pbp2x_motif1_other * 0.256)+
      ...
      {
        sxt <- tibble(sxt_MIC = "Error", sxt_interp = "Error", sxt_profile = "Error")
      }else
      {
        sxt_MIC_calc <- round(-2.265 + (folA$v_folA * 2.113)+ (folP$v_folP * 2.668))
        sxt <- MICcalc(MIC_table, "sxt", sxt_MIC_calc, -1, 4L)
        sxt$sxt_MIC[sxt_MIC_calc < 0] <- "<= 0.5/9.5 ug/ml"
        sxt$sxt_MIC[sxt_MIC_calc > 1] <- ">= 4/76 ug/ml"
      }

      ##### Macrolides: ERY, AZI, CLA, CLI #####
      ery <- NA; azi <- NA; cla <- NA; cli <- NA
      ...
      {
        ery <- tibble(ery_MIC = "Error", ery_interp = "Error", ery_profile = "Error")
        azi <- tibble(azi_MIC = "Error", azi_interp = "Error", azi_profile = "Error")
        cla <- tibble(cla_MIC = "Error", cla_interp = "Error", cla_profile = "Error")
        cli <- tibble(cli_MIC = "Error", cli_interp = "Error", cli_profile = "Error")
      }else
      {
        mefAE$v_mefAE[1] <- ifelse(ermB$lw_ermB[1] == "POS", 0, mefAE$v_mefAE[1])

        ery_MIC_calc <- round(-2.975 +
                             (rRNA23S$v_23S_A2059G * 1.993)+ (ermB$v_ermB * 7.680)+
                             (mefAE$v_mefAE * 4.808))
        ery <- MICcalc(MIC_table, "ery", ery_MIC_calc, -3, 5L)

        azi_MIC_calc <- round(-1.9722 +
                             (rRNA23S$v_23S_A2059G * 1.1204)+ (rRNA23S$v_23S_C2611T * 0.9917)+
                             (ermB$v_ermB * 3.9722)+ (mefAE$v_mefAE * 3.8122))
        azi <- MICcalc(MIC_table, "azi", azi_MIC_calc, -2, 1L)

        cla_MIC_calc <- round(-4.984 +
                             (rRNA23S$v_23S_A2059G * 1.819)+ (rRNA23S$v_23S_C2611T * 1.246)+
                             (ermB$v_ermB * 10.820)+ (mefAE$v_mefAE * 6.064))
        cla <-  MICcalc(MIC_table, "cla", cla_MIC_calc, -5, 6L)
        cla$cla_MIC[cla$cla_MIC == "<= 0.031 ug/ml"] <- "<= 0.03 ug/ml"

        cli_MIC_calc <- round(-2.8145 +
                             (rRNA23S$v_23S_A2059G * 0.456)+ (ermB$v_ermB * 9.048))
      ...
      {
        chl <- tibble(chl_MIC = "Error", chl_interp = "Error", chl_profile = "Error")
      }else if(cat$lw_cat == "POS")
      {
        chl <- tibble(chl_MIC = ">= 8 ug/ml", chl_interp = "Resistant", chl_profile = "CHL-R")
      }else
      {
        chl <- tibble(chl_MIC = "<= 4 ug/ml", chl_interp = "Susceptible", chl_profile = NA)
      }
      ...
      {
        tet <- tibble(tet_MIC = "Error", tet_interp = "Error", tet_profile = "Error")
        dox <- tibble(dox_MIC = "Error", dox_interp = "Error", dox_profile = "Error")
      }else if(tetM$lw_tetM == "POS")
      {
        tet <- tibble(tet_MIC = ">= 16 ug/ml", tet_interp = "Resistant", tet_profile = "TET-R")
        dox <- tibble(dox_MIC = ">= 4 ug/ml", dox_interp = "Resistant", dox_profile = "DOX-R")
      }else
      {
        tet <- tibble(tet_MIC = "<= 1 ug/ml", tet_interp = "Susceptible", tet_profile = NA)
        dox <- tibble(dox_MIC = "<= 0.25 ug/ml", dox_interp = "Susceptible", dox_profile = NA)
      }
      ...
      {
        van <- tibble(van_MIC = ">= 2 ug/ml", van_interp = "Resistant", van_profile = "VAN-R")
      }else
      {
        van <- tibble(van_MIC = "<= 1 ug/ml", van_interp = "Susceptible", van_profile = NA)
      }

      ###################### Combine them for AMR profile ######################
      amr_profile_v <- paste(pen$pen_profile, cro$cro_profile, cxm$cxm_profile,
      ...
      amr_profile_v <- gsub("NA/", "", amr_profile_v)
      amr_profile_v <- gsub("NA", "", amr_profile_v)
      amr_profile_v <- sub("\\/$", "", amr_profile_v)
      amr_profile_v <- ifelse(amr_profile_v == "", "Susceptible", amr_profile_v)
      ...
      sample_data.df <- tibble(lw_CurrSampleNo, pbp1a$lw_pbp1a, pbp2b$lw_pbp2b, pbp2x$lw_pbp2x,
                               ...
                               amr_profile_v, lw_allele_profile)
      names(sample_data.df)[names(sample_data.df) == "amr_profile_v"] <- "amr_profile"
  ...
  write.csv(lw_output.df, paste0(outdir, "LabWareUpload_PNEUMO_AMR.csv"), quote = FALSE, row.names = FALSE)
  write.csv(lw_output_good.df, paste0(outdir, "LabWareUpload_PNEUMO_AMR_good.csv"), quote = FALSE, row.names = FALSE)
  write.csv(lw_output_bad.df, paste0(outdir, "LabWareUpload_PNEUMO_AMR_bad.csv"), quote = FALSE, row.names = FALSE)
  return(lw_output.df)
}
