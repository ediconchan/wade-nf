# ============================================================================
# LABWARE_GONO_AMR -- port of R/LabwareUpload_GONO_AMR.R : labware_gono_amr()
# ----------------------------------------------------------------------------
# Combines three collated profiles for the GONO AMR_ALL chain:
#   amr_profile  = output_profile_GONO_AMR.csv          (MASTERBLASTR AMR loci)
#   ngstar       = output_profile_mut_GONO_NGSTAR.csv   (MLST NGSTAR_AMR mutations)
#   rrna23s      = output_profile_GONO_rRNA23S.csv      (23S counts)
#   refdir       = wade-data/GONO/AMR  (reads reference/MIC_table.csv)
# Predicted MICs via regression + MICcalc; logic kept verbatim. Also writes
# Elevated_MICs.csv. NGSTAR mutation columns are renamed to <locus>_mutations.
# ============================================================================

run_labware_gono_amr <- function(amr_profile, ngstar, rrna23s, refdir, outdir) {
  outdir <- sub("/?$", "/", outdir)
  refdir <- sub("/?$", "/", refdir)
  dir.create(outdir, showWarnings = FALSE, recursive = TRUE)

  AMR_Output.df <- as_tibble(read.csv(amr_profile, header = TRUE, sep = ",", stringsAsFactors = FALSE))
  NGSTAR_Output.df <- as_tibble(read.csv(ngstar, header = TRUE, sep = ",", stringsAsFactors = FALSE))
  ...
  for (m in 1L:NumSamples)
  {
    ...
    } else
    {
      ...
      {
        porB <- tibble(lw_porB = "porB1a", lw_porB_struct = "porB1a",
                       lw_porB_G120 = "NA", lw_porB_A121 = "NA",
                       molec_profile_porB = "NA", v_porB_G120 = 0, v_porB_A121 = 0)
      }else
      {
      ...
      {
        azi <- tibble(azi = "Err", azi_interp = "Err", azi_profile = "AZI-Err")
      }else
      {
        azi_MIC_calc <- round(-3.014+
                             (2.596*rRNA23S$lw_23S_A2059G)+ (1.313*rRNA23S$lw_23S_C2611T)+
                             (2.893*mtrR$v_mtrR_MEN)+ (5.014*mtrR$v_mtrR_Disrupted)+
                             (0.443*mtrR$v_mtrR_35Adel)+ (2.825*ermB$v_ermB)+
                             (4.038*ermC$v_ermC)+ (0.840*porB$v_porB_G120))
      ...
      {
        pen <- tibble(pen = "Err", pen_interp = "Err", pen_profile = "PEN-Err")
      }else
      {
        pen_MIC_calc <- round(-3.21+
                             (6.42*bla$v_bla)+ (0.56*mtrR$v_mtrR_MENDIS)+
                             (1.34*porB$v_porB_G120)+ (1.55*ponA$v_ponA)+
                             (1.25*penA$v_penA_N513Y)+ (1.28*penA$v_penA_A517G)+
                             (0.42*penA$v_penA_G543S))
        pen <- MICcalc(MIC_table, "pen", pen_MIC_calc, -3L, 7L)
      }

      ##### Cephalosporins: CRO, CFM #####
      cro <- NA
      cfm <- NA
      if(str_detect(molec_profile, paste(c("mtrR Err", "porB Err", "ponA Err", "penA Err"),collapse = '|')))
      {
        cro <- tibble(cro = "Err", cro_interp = "Err", cro_profile = "CRO-Err")
        cfm <- tibble(cfm = "Err", cfm_interp = "Err", cfm_profile = "CFM-Err")
      }else
      {
        cro_MIC_calc <- round(-7.72+
                             (0.54*mtrR$v_mtrR_MENDIS)+ (1.38*porB$v_porB_G120)+
                             (0.67*ponA$v_ponA)+ (3.90*penA$v_penA_A311V)+
                             (5.15*penA$v_penA_A501P)+ (1.51*penA$v_penA_A501T)+
                             (1.92*penA$v_penA_A501V)+ (1.53*penA$v_penA_N513Y)+
                             (0.43*penA$v_penA_A517G)+ (0.48*penA$v_penA_G543S))
        cro <- MICcalc(MIC_table, "cro", cro_MIC_calc, -8L, 1L)

        cfm_MIC_calc <- round(-7.198+
                             (0.386*mtrR$v_mtrR_MENDIS)+ (0.932*mtrR$v_mtrR_G45)+
                             (0.407*porB$v_porB_G120)+ (5.422*penA$v_penA_A311V)+
                             (4.494*penA$v_penA_A501P)+ (1.463*penA$v_penA_A501T)+
                             (1.185*penA$v_penA_A501V)+ (4.297*penA$v_penA_N513Y)+
      ...
      {
        cip <- tibble(cip = "Err", cip_interp = "Err", cip_profile = "CIP-Err")
      }else
      {
        cip_MIC_calc <- round(-7.62+
                             (5.67*gyrA$v_gyrA_S91)+ (5.05*parC$v_parC_D86)+
                             (5.67*parC$v_parC_S87R)+ (4.18*parC$v_parC_S87I)+
                             (5.95*parC$v_parC_S87C)+ (1.79*parC$v_parC_S87N)+
                             (1.45*parC$v_parC_S88)+ (5.43*parC$v_parC_E91))
      ...
      if(rpsJ$lw_rpsJ == "Err")
      {
        tet <- tibble(tet = "Err", tet_interp = "Err", tet_profile = "TET-Err")
        tetfail <- TRUE
      }
      ...
      if(tetfail == FALSE)
      {
        ...
        }else
        {
          tet_MIC_calc <- round(-1.83+
                               (0.62*mtrR$v_mtrR_ANY)+ (0.26*mtrR$v_mtrR_A39)+
                               (0.79*porB$v_porB_G120)+ (0.22*porB$v_porB_A121)+
                               (2.11*rpsJ$v_rpsJ)+ (4.15*tetM$v_tetM))
      ...
      ##### Spectinomycin (SPE) #####
      spe <- NA
      spe <- tibble(spe = "<= 32 ug/ml", spe_interp = "Susceptible", spe_profile = "")
      if(str_detect(molec_profile, "16S rRNA Err/Err"))
      {
        spe <- tibble(spe = "Err", spe_interp = "Err", spe_profile = "SPE-Err")
      }else
      {
        if(rRNA16S$lw_16S_C1192T == "C1192T")
        {
          spe <- tibble(spe = ">= 128 ug/ml", spe_interp = "Resistant", spe_profile = "SPE-R")
        }
        if(rRNA16S$lw_16S_T1458C == "T1485C")
        {
          spe <- tibble(spe = "64 ug/ml", spe_interp = "Resistant", spe_profile = "SPE-R")
        }
      }
  ...
  write.csv(lw_Output.df, paste0(outdir, "LabWareUpload_GONO_AMR.csv"), quote = FALSE, row.names = FALSE)
  write.csv(lw_output_good.df, paste0(outdir, "LabWareUpload_GONO_AMR_good.csv"), quote = FALSE, row.names = FALSE)
  write.csv(lw_output_bad.df, paste0(outdir, "LabWareUpload_GONO_AMR_bad.csv"), quote = FALSE, row.names = FALSE)

  highMICs <- lw_Output.df %>% mutate_all(function(x) gsub(" ug/ml|>=|<=","",x))
  ...
  write.csv(highMICs, paste0(outdir, "Elevated_MICs.csv"), quote = FALSE, row.names = FALSE)
  return(lw_Output.df)
}
